...
| 원칙 | 설명 |
|------|------|
| **소통 우선** | 주요 코드 수정, 빌드, 커밋, 푸시, 태그 생성, 릴리스 등 외부 영향이 있는 모든 작업 수행 전 반드시 사용자에게 다음 단계를 설명하고 승인을 받아야 함 (독단적 진행 절대 금지) |
| **Git Reset 금지** | **에이전트는 git reset (특히 git reset --hard) 명령어를 절대로 실행해서는 안 됨.** 작업 내역 철회나 되돌리기가 필요할 경우 오직 `git revert` 또는 코드 단위의 정밀 복구만 수행해야 함 |
